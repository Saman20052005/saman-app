...
**Document Version:** 3.7 (Comprehensive Design System Alignment, Canvas Artifact Mapping & Active Tracker Architecture)  
...
#### 5.1 Home Screen Architecture (Primary Surface — `{{DATA:SCREEN:SCREEN_44}}`)
...
- **B. Workout Recommendation Carousel:**
  ...
  - **Dominant Card:** Moody editorial lifting visual (`{{DATA:IMAGE:IMAGE_46}}`), recovery rationale badge (*"Best match for your recovery"*), workout title (*"Leg Day & Posterior Chain"*), specifications (*"45 min · 5 compound exercises · Target RPE 8.5"*), exercise chips, and high-contrast solid CTA button (*"Start Workout →"*).
  - **Secondary Alternatives:** *Upper Strength* (40 min, `{{DATA:IMAGE:IMAGE_43}}`), *Full Body Express* (25 min, `{{DATA:IMAGE:IMAGE_42}}`), and *Active Recovery* (20 min, `{{DATA:IMAGE:IMAGE_41}}`) with minimal *"View workout →"* action.
...
- **F. Saman Picks (Contextual Surface):**
  ...
  - Card 2: **Saman Shop** (Curated training essentials—matte straps `{{DATA:IMAGE:IMAGE_40}}`, shaker bottle—strictly free of prices, sales badges, or checkout carts).
...
1. **Empty / New Conversation State (`{{DATA:SCREEN:SCREEN_32}}`):**
...
2. **Active Conversation State (`{{DATA:SCREEN:SCREEN_30}}`):**
...
3. **Food Photo Analysis Loading State (`{{DATA:SCREEN:SCREEN_17}}`):**
...
4. **Food Photo Result State (`{{DATA:SCREEN:SCREEN_27}}`):**
   - High-fidelity meal photograph thumbnail (`{{DATA:IMAGE:IMAGE_29}}`).
...
5. **Reminder / Proactive Nudge State (`{{DATA:SCREEN:SCREEN_23}}`):**
...
6. **Long Response / Markdown / Table State (`{{DATA:SCREEN:SCREEN_11}}`):**
...
7. **Loading & Typing State (`{{DATA:SCREEN:SCREEN_19}}`):**
...
8. **Recoverable Error State (`{{DATA:SCREEN:SCREEN_15}}`):**
...
9. **Food Photo Failure State (`{{DATA:SCREEN:SCREEN_13}}`):**
...
#### 5.3 Saman Coach Full-Height Drawer (Personal Coach Workspace — `{{DATA:SCREEN:SCREEN_9}}`)
