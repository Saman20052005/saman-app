...
**Document Version:** 3.8 (Comprehensive Multi-Surface Specification, Design System Architecture & In-Gym Tracker Milestone)  
...
5. **Prepares for focused gym execution** with an in-development active workout tracker centered on set-by-set compound lifting, automated plate math, and live RPE capture.
...
#### 5.1 Home Screen Architecture (Primary Surface — `{{DATA:SCREEN:SCREEN_45}}`)
...
- **B. Workout Recommendation Carousel:**
  ...
  - **Dominant Card:** Moody editorial lifting visual (`{{DATA:IMAGE:IMAGE_47}}`), recovery rationale badge (*"Best match for your recovery"*), workout title (*"Leg Day & Posterior Chain"*), specifications (*"45 min · 5 compound exercises · Target RPE 8.5"*), exercise chips, and high-contrast solid CTA button (*"Start Workout →"*).
  - **Secondary Alternatives:** *Upper Strength* (40 min, `{{DATA:IMAGE:IMAGE_44}}`), *Full Body Express* (25 min, `{{DATA:IMAGE:IMAGE_43}}`), and *Active Recovery* (20 min, `{{DATA:IMAGE:IMAGE_42}}`) with minimal *"View workout →"* action.
...
- **F. Saman Picks (Contextual Surface):**
  ...
  - Card 2: **Saman Shop** (Curated training essentials—matte straps `{{DATA:IMAGE:IMAGE_41}}`, shaker bottle—strictly free of prices, sales badges, or checkout carts).
...
1. **Empty / New Conversation State (`{{DATA:SCREEN:SCREEN_33}}`):**
...
2. **Active Conversation State (`{{DATA:SCREEN:SCREEN_31}}`):**
...
3. **Food Photo Analysis Loading State (`{{DATA:SCREEN:SCREEN_18}}`):**
...
4. **Food Photo Result State (`{{DATA:SCREEN:SCREEN_28}}`):**
   - High-fidelity meal photograph thumbnail (`{{DATA:IMAGE:IMAGE_30}}`).
...
5. **Reminder / Proactive Nudge State (`{{DATA:SCREEN:SCREEN_24}}`):**
...
6. **Long Response / Markdown / Table State (`{{DATA:SCREEN:SCREEN_12}}`):**
...
7. **Loading & Typing State (`{{DATA:SCREEN:SCREEN_20}}`):**
...
8. **Recoverable Error State (`{{DATA:SCREEN:SCREEN_16}}`):**
...
9. **Food Photo Failure State (`{{DATA:SCREEN:SCREEN_14}}`):**
...
#### 5.3 Saman Coach Full-Height Drawer (Personal Coach Workspace — `{{DATA:SCREEN:SCREEN_10}}`)
...
- **Phase 3 (Active In-Gym Workout Tracker — Immediate Priority):**  
