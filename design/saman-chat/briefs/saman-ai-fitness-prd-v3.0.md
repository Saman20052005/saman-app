# Product Requirements Document (PRD) & Comprehensive Product Brief
## Saman — AI-Driven Personal Fitness, Nutrition & Health Companion

**Document Version:** 3.0 (Post-Coach Conversation, State Architecture & Workspace Drawer Release)  
**Status:** Approved / Active Production Specification  
**Target Platform:** Native Mobile (iOS & Android — ~390px mobile viewport)  
**Product Category:** Health & Fitness, Contextual AI Coaching, Strength & Performance  
...
In sharp contrast to legacy fitness applications that overwhelm athletes with "dashboard fatigue"—cluttered metric rings, noisy charts, intrusive marketing popups, and gimmick chatbot widgets—Saman grounds its entire user experience in **calm athleticism**. 

The app functions as an intuitive, authoritative in-gym training partner that:
1. **Presents the primary daily decision upfront** (today’s programmed workout session).
2. **Maintains disciplined daily nutrition guardrails** (flat macro tracking with semantic color discipline).
...
3. **Calm Athleticism (Dark Obsidian Aesthetic):**
   ...
   - **Disciplined Color Accents:** Strict monochrome foundation. Accents are reserved exclusively for data semantics:
     - **Saman Green (`#10b981`):** Completed workouts, active streaks, protein progress, active reminder dot.
...
4. **Food Photo Result State:**
   - High-fidelity meal photograph thumbnail (`{{DATA:IMAGE:IMAGE_22}}`).
