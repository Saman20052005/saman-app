...
**Document Version:** 3.3 (Comprehensive Production Specification & Implementation Baseline)  
...
- **B. Workout Recommendation Carousel:**
  ...
  - **Dominant Card:** Moody editorial lifting visual (`{{DATA:IMAGE:IMAGE_42}}`), recovery rationale badge (*"Best match for your recovery"*), workout title (*"Leg Day & Posterior Chain"*), specifications (*"45 min · 5 compound exercises · Target RPE 8.5"*), exercise chips, and high-contrast solid CTA button (*"Start Workout →"*).
  - **Secondary Alternatives:** *Upper Strength* (40 min, `{{DATA:IMAGE:IMAGE_39}}`), *Full Body Express* (25 min, `{{DATA:IMAGE:IMAGE_38}}`), and *Active Recovery* (20 min, `{{DATA:IMAGE:IMAGE_37}}`) with minimal *"View workout →"* action.
...
- **F. Saman Picks (Contextual Surface):**
  ...
  - Card 2: **Saman Shop** (Curated training essentials—matte straps `{{DATA:IMAGE:IMAGE_36}}`, shaker bottle—strictly free of prices, sales badges, or checkout carts).
...
4. **Food Photo Result State:**
   - High-fidelity meal photograph thumbnail (`{{DATA:IMAGE:IMAGE_25}}`).
...
- **Phase 3 (Active In-Gym Workout Tracker — Next Priority):**  
