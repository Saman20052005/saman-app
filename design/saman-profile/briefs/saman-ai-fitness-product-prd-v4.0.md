...
**Document Version:** 4.0 (Comprehensive Multi-Surface Specification, Profile Architecture, Conversational Intelligence & Active In-Gym Tracker Milestone)  
...
Unlike legacy fitness applications that suffer from **dashboard fatigue**—overwhelming users with disconnected metric rings, cluttered charts, aggressive gamification, and intrusive upsells—Saman grounds its entire user experience in **calm athleticism**. 

The product acts as an authoritative in-gym training partner that:
1. **Presents the primary daily decision upfront** through today’s programmed workout session on the Home feed.
...
5. **Establishes a private personal fitness hub** via the **Profile** surface—replacing mixed administrative dashboards and editable form fields with a clean, read-oriented, and navigational overview of long-term progress, biometrics, goals, and account preferences.
6. **Prepares for focused gym execution** with an in-development active workout tracker centered on set-by-set compound lifting, automated plate math, and live RPE capture.
...
5. **Cluttered Administrative Profiles:** Standard profile pages often mix account authentication, editable body inputs, medical notes, and global "Save" buttons into messy forms instead of acting as an inspiring, private personal health hub.
...
3. **Calm Athleticism (Dark Obsidian Aesthetic):**
   ...
   - **Card & Panel Surfaces:** Dark charcoal (`#16171b` / `#1b1b1f`) framed by hairline graphite borders (`#23252a` or `#26282f` / `rgba(255,255,255,0.08)`).
   - **Typography Hierarchy:** Tabular, high-contrast white numbers, clean title-case headers (`font-medium`), and muted graphite secondary copy (`#94979e` / `#71717a`). Monospace is strictly reserved for data timestamps where necessary.
   - **Disciplined Color Accents:** Strict monochrome foundation. Accents are reserved exclusively for data semantics:
     ...
     - **Muted Red (`#ef4444` / `#f87171`):** Understated text-only destructive actions (e.g., Log out).
     - **Strict Negatives:** No neon, no electric blue CTAs, no decorative gradients, no heavy glassmorphism, no gold VIP badges.
...
5. **Profile:** Clean contoured person silhouette with an active off-white micro-dot indicator when selected.

---

### 5. Detailed Screen Specifications & Live Canvas Artifacts

#### 5.1 Home Screen Architecture (Primary Surface — `{{DATA:SCREEN:SCREEN_52}}`)
...
- **B. Workout Recommendation Carousel:**
  ...
  - **Dominant Card:** Moody editorial lifting visual (`{{DATA:IMAGE:IMAGE_54}}`), recovery rationale badge (*"Best match for your recovery"*), workout title (*"Leg Day & Posterior Chain"*), specifications (*"45 min · 5 compound exercises · Target RPE 8.5"*), exercise chips, and high-contrast solid CTA button (*"Start Workout →"*).
  - **Secondary Alternatives:** *Upper Strength* (40 min, `{{DATA:IMAGE:IMAGE_51}}`), *Full Body Express* (25 min, `{{DATA:IMAGE:IMAGE_50}}`), and *Active Recovery* (20 min, `{{DATA:IMAGE:IMAGE_49}}`) with minimal *"View workout →"* action.
...
- **F. Saman Picks (Contextual Surface):**
  ...
  - Card 2: **Saman Shop** (Curated training essentials—matte straps `{{DATA:IMAGE:IMAGE_48}}`, shaker bottle—strictly free of prices, sales badges, or checkout carts).
...
1. **Empty / New Conversation State (`{{DATA:SCREEN:SCREEN_40}}`):**
...
2. **Active Conversation State (`{{DATA:SCREEN:SCREEN_38}}`):**
...
3. **Food Photo Analysis Loading State (`{{DATA:SCREEN:SCREEN_25}}`):**
...
4. **Food Photo Result State (`{{DATA:SCREEN:SCREEN_35}}`):**
   - High-fidelity meal photograph thumbnail (`{{DATA:IMAGE:IMAGE_37}}`).
...
5. **Reminder / Proactive Nudge State (`{{DATA:SCREEN:SCREEN_31}}`):**
...
6. **Long Response / Markdown / Table State (`{{DATA:SCREEN:SCREEN_19}}`):**
...
7. **Loading & Typing State (`{{DATA:SCREEN:SCREEN_27}}`):**
...
8. **Recoverable Error State (`{{DATA:SCREEN:SCREEN_23}}`):**
...
9. **Food Photo Failure State (`{{DATA:SCREEN:SCREEN_21}}`):**
...
#### 5.3 Saman Coach Full-Height Drawer (Personal Coach Workspace — `{{DATA:SCREEN:SCREEN_17}}`)
...
#### 5.4 Profile Screen Architecture (Personal Fitness Hub — `{{DATA:SCREEN:SCREEN_3}}`)

Accessed via the 5th tab (**Profile**) on the persistent bottom navigation bar.

- **Purpose & Feeling:** Designed as a private, calm athletic fitness hub rather than a social network profile, administrative dashboard, SaaS settings menu, or onboarding form. Completely read-oriented and navigational; detailed edits occur in dedicated downstream sub-screens.
- **A. Top Header:**
  - Standard status bar (`9:41`).
  - Left-aligned primary title: **Profile** (`text-2xl font-bold text-white`).
  - Subtitle: `Your health, progress & account` (`text-sm text-[#94979E]`).
  - Right-aligned compact square Settings gear button (`w-10 h-10 rounded-xl bg-[#1B1D22] border border-[#26282F]`), matching Home header button styling.
  - Strict exclusions: No back arrow (primary tab), no theme switch in header.
- **B. Personal Identity Card:**
  - Matte charcoal card (`#16171B`) with hairline border (`#26282F`).
  - Circular avatar container (`52×52px`) with user initials (`NA`).
  - Athlete credentials: Name (**Nguyễn Văn An** in `text-lg font-bold`), safe-truncating email (`saman2552988@gmail.com`), compact `FREE PLAN` pill badge, and `Member since 2026`.
  - Compact circular edit profile action button (`w-8 h-8 rounded-full bg-[#26282F]`).
  - Strict exclusions: No follower counts, no likes, no social posts, no BMI beside email.
- **C. Progress Snapshot Module (`YOUR PROGRESS`):**
  - Section label: `YOUR PROGRESS` in crisp uppercase micro-typography (`text-xs font-semibold tracking-wider text-[#94979E]`).
  - Single unified matte card inspired by Strava and Hevy clarity, styled strictly within Saman obsidian discipline.
  - 3 long-term consistency metrics with subtle vertical separators:
    - **24** — Workouts
    - **7d** — Current streak (highlighted in semantic Saman Green `#10B981`)
    - **82%** — Plan adherence
  - Footer link: `View progress →` (humanized copy; no technical telemetry jargon).
- **D. Saman+ Membership Opportunity:**
  - Understated premium card with subtle emerald brand accent dot (`• SAMAN+`).
  - Title: **Unlock more with Saman+**.
  - Supporting copy: *"Adaptive plans, deeper insights and more personalized coaching."*
  - Benefits line: `Advanced reports · Body trends · More Saman Coach`.
  - Action link: `Explore Saman+ →`.
  - Guardrails: No gold VIP styling, crowns, diamonds, prices, countdown timers, or aggressive upgrade banners.
- **E. Health & Goals Summary (Grouped Navigational Rows):**
  - Section label: `HEALTH & GOALS`.
  - Grouped settings card with thin dividers and responsive two-line layout for long entries:
    - **Body profile**: Line 1: `Body profile` | Line 2: `170 cm · 70 kg · BMI 24.2` + chevron.
    - **Primary goal**: Single-line `Primary goal` · `Build muscle` + chevron.
    - **Activity level**: Single-line `Activity level` · `High` + chevron.
    - **Nutrition targets**: Line 1: `Nutrition targets` | Line 2: `2,450 kcal · 160g protein` + chevron.
- **F. Personal Information & Privacy:**
  - Grouped rows continuing calm privacy boundaries:
    - **Medical information**: `Medical information` · `Private` with discreet lock icon + chevron.
    - **Measurements & photos**: Line 1: `Measurements & photos` | Line 2: `Track body progress` + chevron.
- **G. Preferences:**
  - Section label: `PREFERENCES`.
  - Grouped card with uniform dark icon containers (`#212328`):
    - **Language**: `English` + chevron.
    - **Units**: `Metric` + chevron.
    - **Appearance**: `Dark Obsidian` + chevron.
    - **Notifications**: `Active` + chevron.
    - **Connected apps**: Line 1: `Connected apps` | Line 2: `Apple Health · Whoop` + chevron.
- **H. Account & Support & Logout:**
  - Section label: `ACCOUNT & SUPPORT`.
  - Grouped card: *Manage membership*, *Privacy & security*, *Export data log*, *Help & feedback*, *About Saman (v3.8)*.
  - **Quiet Destructive Action:** Text-only `Log out` row in muted red (`#ef4444`) with exit icon, positioned below the card with generous bottom scroll clearance (`pb-32` + `pb-8` ~ 140px) ensuring complete visibility above the fixed bottom navigation bar.
...
4. **Local-First Caching & Resilience:**
   - SQLite client database on-device for full offline workout logging, profile caching, and meal history.
...
| **Profile Goal Recalibration Rate** | > 35% monthly | Confirms athletes treat Profile as an active, living personal fitness hub |
...
- **Phase 2 (Saman Coach & Conversational Intelligence — Delivered & Baselined):**  
...
- **Phase 3 (Profile & Personal Fitness Hub — Delivered & Baselined):**  
  Private, read-oriented personal health hub (Athlete identity, 3-metric progress snapshot, quiet Saman+ membership, responsive two-line health & goals summaries, privacy-guarded medical rows, preferences, and account support with 140px+ scroll clearance).
- **Phase 4 (Active In-Gym Workout Tracker — Immediate Priority):**  
  Set-by-set compound lifting execution mode with automatic barbell plate math, rest countdown timers, live RPE capture, and direct sync to recovery metrics.
- **Phase 5 (Deep Wearable & Recovery Periodization):**  
