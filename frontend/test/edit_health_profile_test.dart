...
void main() {
  ...
  group('EditHealthProfileScreen - Allergies Exclusion Contract', () {
    ...
    test('ProfileRemoteDataSourceImpl sends toUpdateJson without allergies',
      ...
      final snapshot = await dataSource.updateProfile(model);

      expect(snapshot, isA<ProfileSnapshotModel>());
  ...
  group('EditHealthProfileScreen - Production Save Error & Success Contract',
    ...
    testWidgets(
        'save success applies canonical snapshot directly with 0 GET refresh and pops',
        (tester) async {
      setMobileView(tester);
      final repo = _MockProfileRepository();
      ...
      // Exactly 1 sync (POST) and 0 refresh GET were executed
      expect(repo.syncCount, equals(1));
      expect(repo.fetchCount, equals(0));
  ...
  group('ProfileNotifier - POST Success without GET Refresh', () {
    testWidgets(
        'when syncProfile succeeds, updateProfile applies snapshot directly without calling fetchProfileSnapshot',
        (tester) async {
      setMobileView(tester);
      ...
      expect(endState.status, equals(ProfileStatus.ready));
      expect(endState.profile.age, equals(26));
    });
  });
}
