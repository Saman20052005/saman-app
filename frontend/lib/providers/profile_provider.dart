...
class ProfileNotifier extends StateNotifier<ProfileState> {
  ...
  Future<void> updateProfile(ProfileEntity newProfile) async {
    final saveEpoch = _sessionEpoch;
    state = state.copyWith(status: ProfileStatus.loading);

    try {
      final result = await _repository.syncProfile(newProfile);
      final ProfileSnapshot snapshot = result is ProfileSnapshot
          ? result
          : ProfileSnapshot(profile: newProfile);

      // Epoch revalidation after POST completion
      if (saveEpoch != _sessionEpoch || !mounted) {
        debugPrint(
            '[PROFILE] Discarding stale update response from epoch $saveEpoch (current: $_sessionEpoch)');
        return;
      }

      // Revalidate before cache write & await cache write
      await _repository.cacheSnapshot(snapshot);

      // Epoch revalidation after awaited cache write & before applying state
      if (saveEpoch != _sessionEpoch || !mounted) {
        debugPrint(
            '[PROFILE] Discarding state application from epoch $saveEpoch after update cache write (current: $_sessionEpoch)');
        return;
      }

      _applySnapshot(snapshot);
    } on UnauthorizedException catch (e) {
      if (saveEpoch != _sessionEpoch || !mounted) return;
      debugPrint('[PROFILE] Unauthorized on update (401)');
      state = ProfileState(
        status: ProfileStatus.unauthorized,
        profile: ProfileEntity.empty(),
        errorMessage: e.message,
      ...
      if (state.profile.isValid) {
        state = state.copyWith(
          status: state.isFromCache ? ProfileStatus.offline : ProfileStatus.ready,
          errorMessage: e.toString(),
        );
      } else {
        state = state.copyWith(
  ...
  void _applySnapshot(ProfileSnapshot snapshot) {
    ...
    state = ProfileState(
      ...
      email: snapshot.email ?? state.email,
      fullName: snapshot.fullName ?? state.fullName,
      avatar: snapshot.avatar ?? state.avatar,
      isFromCache: isCache,
    );
  }
